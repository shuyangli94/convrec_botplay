...
from convrec.utils import df_to_csv, get_last_mod_ckpt, save_config
...
def train_model(config, gpus, multi_gpus: bool, system_class):
    ...
    dm_kwargs = dict(
        ...
        item_as_kp=getattr(cfg, "item_as_kp", False),
        shuffle_train=True,
        use_user_kp=getattr(cfg, "use_user_kp", False),
    )
    ...
    if cfg.load:
        ...
        model = system_class.load_from_checkpoint(
            ...
            all_item_kp=dm.item_kp_vec,
            strict=False,
        )
    else:
        print("!!!!!!    INITIALIZING FROM SCRATCH     !!!!!!!!!!")
        model = system_class(cfg, train_ui=dm.train_ui, all_item_kp=dm.item_kp_vec)
    ...
    if (
        ...
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
            ...
            mode="max" if choice_metric in {"auc", "NDCG@50"} else "min",
            dirpath=save_path,
            save_last=True,
    ...
    mdf = pd.read_csv(model.metrics_loc, names=model.saved_metrics).sort_values(
        [choice_metric, "loss"],
        ascending=[choice_metric not in {"auc", "NDCG@50", "hr@10", "hr@20"}, True],
    )
    print("\n\n")
...
def main(cfg: DictConfig) -> None:
    ...
    total_grid = 1
    hparams = []
    for hyperparam in ["batch_size", "kp_weight", "lr", "l2_lambda", "target_item"]:
        candidates = []
    ...
    for model_hps in product(*model_hp):
        ...
        for train_hps in product(*hparams):
            ...
            for train_hp_name, train_hp_val in train_hps:
                ...
                if train_hp_name in {"target_item"}:
    ...
    try:
        mdf = mdf.sort_values(
            [metric_name, "loss"],
            ascending=[
                metric_name not in {"auc", "NDCG@50", "hr@10", "hr@20", "acc"},
                True,
            ],
        )
        print("\n\nTOTAL STATS")
