...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-ft", action="store_true", default=False)
    parser.add_argument("--cfg-loc", type=str, default=None)
    ...
    if args.no_ft and not args.cfg_loc:
        raise NotImplementedError("Must specify `--cfg-loc` if `--no-ft` is specified!")
    elif not args.no_ft and args.cfg_loc:
        raise NotImplementedError(
            "`--cfg-loc` is not active if `--no-ft` is NOT specified - please correct!"
        )
    ...
    if args.no_ft:
        numpy_loc = os.path.join(model_dir, "model.pt")
        model = PLRecWithAspectsKPTorch.from_pretrained_numpy(numpy_loc)
    else:
        ...
        _, model = load_model(ckpt_path=ckpt, cfg=cfg, strict=True)
    model.eval()
    ...
    print(
        "Loaded {} with {:,} params".format(
            model.__class__.__name__, count_parameters(model)
        )
    )
