...
from convrec.models import BPRUserKP
...
class System(pl.LightningModule):
    ...
    def __init__(self, config, train_ui: dict = None, **kwargs):
        super().__init__()
        ...
        # Aspect target (default: (u, i) review)
        self.target_item_aspects = getattr(self.config, 'target_item', False)
        if self.target_item_aspects:
            print('>> Predicting ITEM aspects')
        else:
            print('>> Predicting REVIEW aspects')
        ...
        self.model_kwargs = {
            ...
            'enc_layers': getattr(self.config.model_type, 'enc_layers', 0),
            'proj_layers': getattr(self.config.model_type, 'proj_layers', -1),
            'user_emb': getattr(self.config.model_type, 'user_emb', False)
        }
        if self.config.kp_weight == 0:
            print(
                '\n\n----\nRUNNING WITH n_kp = 0 -> NO ASPECT PREDICTION!\n----\n\n'
            )
            self.model_kwargs['proj_layers'] = -1
        print('Creating BPRUserKP model with kwargs:\n{}'.format(
            json.dumps(self.model_kwargs, indent=2, default=str)))
        self.model = BPRUserKP(**self.model_kwargs)
    ...
    def training_step(self, batch, batch_idx, is_eval: bool = False):
        ...
        # aspects -> [B, Na]
        # I/J/U aspects -> [B, Na]
        u, i, j, aspects, i_kp, j_kp, u_kp = batch
        if self.target_item_aspects:
            aspect_targets = i_kp
        else:
            aspect_targets = aspects
        ...
        try:
            ...
            x_ui, x_uj, aspect_ui, bpr_loss, aspect_loss = self.model(
                u=u, i=i, j=j, target_kps=aspect_targets, user_kps=u_kp)
            ...
            debug_log(aspect_targets, 'aspect_targets', debug=True)
            debug_log(u_kp, 'u_kp', debug=True)
            raise
        ...
        if is_eval:
            ...
            if self.config.kp_weight != 0:
                ...
                n_pos_true = (
                    aspect_targets == 1).sum(dim=-1).detach().cpu().numpy()
                n_pos_pred = (
            ...
            item_scores, n_candidates = self.model.get_scores(
                u=u, user_kps=u_kp, invalid=invalid)
