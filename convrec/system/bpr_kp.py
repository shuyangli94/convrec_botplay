...
from convrec.models import BPRKP
...
class System(pl.LightningModule):
    ...
    def __init__(self, config, train_ui: dict = None, **kwargs):
        ...
        self.model_kwargs = {
            ...
            'kp_layers': getattr(self.config.model_type, 'kp_layers', 0),
            'beta': not getattr(self.config.model_type, 'no_bias', False),
        }
        if self.config.kp_weight == 0:
            print(
                '\n\n----\nRUNNING WITH n_kp = 0 -> NO ASPECT PREDICATION!\n----\n\n'
            )
            self.model_kwargs['n_kp'] == 0
        print('Creating BPRKP model with kwargs:\n{}'.format(
            json.dumps(self.model_kwargs, indent=2, default=str)))
        self.model = BPRKP(**self.model_kwargs)
    ...
    def training_step(self, batch, batch_idx, is_eval: bool = False):
        ...
        if is_eval:
            ...
            if self.config.kp_weight != 0:
                ...
                n_pos_true = (aspect_targets == 1).sum(dim=-1).detach().cpu().numpy()
                n_pos_pred = (
            ...
            item_ranks = self.model.get_ranks(
                u, item_scores, index_by_item=True)
